...
module.exports = {
	 ...
	 *
	 * note: This is "version 3". Encryption of previous versions is no longer supported, however 0.45 and 0.51 are both supported
	 * version: string "0.45" | "0.51"
	 */
	...
	decrypt(input, version) {
		...
		if (coreVersion === 1) {
			...
			if (version === "0.51") {
				...
				if (input[input.length - 1] !== makeIntegrityByte1(byte) && input[input.length - 1] !== makeIntegrityByte2(byte)) {
		...
		let unshuffleFn = coreVersion === 1 ? unshuffle : unshuffle2;
